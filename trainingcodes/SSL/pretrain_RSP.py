"""
Resolution Sequence Prediction (RSP) based Self-Supervised Pretraining
"""
import argparse
import os
import time
...
def parse_args():
    ...
    parser.add_argument('--seed', type=int, default=2022, help='seed for initializing training.')

    # model definition
    parser.add_argument('--model', type=str, default='resnet34', help='choice of network architecture - resnet18.')
    parser.add_argument('--num_classes', type=int, default=6, help='# of classes for resolution sequence prediction.')
    parser.add_argument('--num_epoch', type=int, default=300, help='epochs to train for - 300.')
    parser.add_argument('--batch_size', type=int, default=256, help='batch_size - 512.')
    ...
    # Data paths
    parser.add_argument('--train_image_pth', default='SSL_training/images/',
                        help='path to train images WSIs')
    parser.add_argument('--output_pth', default='/home/ramanav/projects/rrg-amartel/Digital_Pathology/Public_Datasets/tiger-training-data/Results/SSL_34/output',
                        help='path to save tiles for visualization')
    parser.add_argument('--model_save_pth', type=str,
                        default='/home/ramanav/projects/rrg-amartel/Digital_Pathology/Public_Datasets/tiger-training-data/Results/SSL_34/Save_Results/',
    ...
    parser.add_argument('--loc_mod',type=str,required=True,help="Data Location Modifier for Compute Canada")
    ...
    parser.add_argument('--tile_stride_w', default=256*3, type=int, help='stride width dx @lowest resolution')
    parser.add_argument('--tile_stride_h', default=256*3, type=int, help='stride height dy @lowest resolution')
    parser.add_argument('--lwst_level_idx', default=5, type=int, help='Select Lowest level for patch indexing in Tiger_Challenge dataset')

    # Augmentation parameters (these parameters are chosen based on ablation studies. Can be experimented with different values)
    parser.add_argument('--NAug', default=2, type=int, help='Number of augmentations to apply sequentially')
    parser.add_argument('--Magn', default=3, type=int, help='Magnitude for all the transformations')
...
def main():
    ...
    if args.loc_mod is None:
        data_path = args.train_image_pth
    else:
        data_path = str(Path(args.loc_mod)/Path(args.train_image_pth))

    # Set the loader
    train_dataset = DatasetWSIs(data_path, args.output_pth, args.tile_h, args.tile_w, args.tile_stride_h,
                                args.tile_stride_w, args.lwst_level_idx, args.NAug, args.Magn)

    # Train and validation split (80/20% ratio)
    train_dataset, val_dataset = random_split(train_dataset, [round((len(train_dataset))*0.8), round((len(train_dataset))*0.2)],
    ...
    in_features = 256  # Feature dimension at the output of pairwise feature extraction stage (256 * 3)
