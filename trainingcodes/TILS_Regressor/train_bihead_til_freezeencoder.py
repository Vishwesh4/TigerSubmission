"""
Trains Resnet bihead network for predicting TIL density in stromal tissue and Tumor Associated Stroma tissue density given
a patch. The encoder is frozen and only the heads are trained on TILs_Patches_v2 dataset
Modified:
    1) Changed the model to Resnet_bihead_v2, inorder to incorporate dropouts
    2) Modified Jun2, changed few variable names, load_model_weights and tried to ensure its up to date
    3) Modified Jun 6, threshold based on roi area and sampling weight changed
"""
import torch, os
import torch._utils
...
from utils.dataloader import TILS_dataset_Bihead_Area_Old
...
def train(model,trainloader, optimizer, LossFun,MetricFun):
    ...
    for data in tqdm(trainloader):
        ...
        if torch.cuda.device_count() > 1 and multi_gpu:
            ...
            cell_outputs,tissue_outputs = outputs
...
if __name__=="__main__":
    ...
    transform_train =  albumentations.Compose([    
        ...
        albumentations.HueSaturationValue(hue_shift_limit=0.5, sat_shift_limit=0.5, val_shift_limit=0.5,p=0.4),
        albumentations.Affine(scale=(0.8,1.2),translate_percent=(0,0.3),shear=(-10,10),p=0.3),
        ToTensorV2()])
    # transform_train =  albumentations.Compose([
    #     ToTensorV2()]) 
    ...
    til = til.loc[(til["class_1"]<3000)].reset_index(drop=True)
    ...
    til.loc[til["sample"]==1,"weight"] = 1/(til["sample"]==1).sum()
    til.loc[til["sample"]==2,"weight"] = 1/(til["sample"]==2).sum()
    ...
    trainset = TILS_dataset_Bihead_Area_Old(data_file=train_dataset,labels=[0,1,5,6],labels_tils=[1,2],path=dataset_path,transform=transform_train)
    ...
    testset = TILS_dataset_Bihead_Area_Old(data_file=test_dataset,labels=[0,1,5,6],labels_tils=[1,2],path=dataset_path,transform=transform_test)
    ...
    for epoch in range(epoch_start,data_hyp['EPOCHS']):
        ...
        if is_save and (((epoch+1) % 6 == 0) or (metric>best_metric)):
